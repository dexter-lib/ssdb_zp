...
hashslot_init(struct server_pool *pool, uint32_t nserver){
    ...
    if (!pool->zh_handler) {
        ...
        snprintf(zk_path, path_len, "/twemproxy/%s/slot_map", (char *)pool->product_prefix.data);
    ...
    for (slot_index = 0; slot_index < HASHSLOT_SLOT_NUM; slot_index++) {
        ...
        if (zk_check_flag) {
            memset(data, 0, datalen);
            memset(zk_path, 0, pathlen);
            sprintf(zk_path, "/twemproxy/%s/slot_map/%d", (char *)pool->product_prefix.data, slot_index);
