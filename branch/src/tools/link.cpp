...
Link* Link::listen(const char *ip, int port){
    ...
    if(::setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO,  (void *) &tv_timeout, sizeof(struct timeval)) == -1){
        goto sock_err;
    }
...
Link* Link::accept(){
    ...
    tv_timeout.tv_sec = 3;
    tv_timeout.tv_usec = 0;
    if(::setsockopt(client_sock, SOL_SOCKET, SO_RCVTIMEO,  (void *) &tv_timeout, sizeof(struct timeval)) == -1){
    }
