...
static struct option long_options[] = {
    ...
    { "product_prefix", required_argument,  NULL,   'P' },
    { NULL,             0,                  NULL,    0  }
};
...
void 
migarte_status(const zhandle_t *zh_handler, int nodes_num, const char *status,const std::vector<std::string> &vecSlots, const char *product_prefix){
	zhandle_t * zk_handler = (zhandle_t *)zh_handler;
	char zk_path[ZK_PATH_LEN] = {0};
	char data[256] = {0};
	...
	for (int i = 0; i < count;i++)
	{
		...
		sprintf(zk_path, "/twemproxy/%s/migrate_tasks/%s", product_prefix, vecSlots[i].c_str());
...
int 
add_node(const zhandle_t *zh_handler, const char *masternode, const char *product_prefix, const char *backupnode=NULL){
	...
	if (zh_handler){
		...
		int  zk_path_len = sizeof(zk_path);
		zk_path[zk_path_len - 1] = '\0';
		snprintf(zk_path, zk_path_len - 1, "/twemproxy/%s/nodes", product_prefix);
	    int child_ret = zk_get_children(zk_handler, zk_path, NULL, NULL, &strings);
	    if (child_ret) {
	    ...
	    for (int i = 0; i < nodes_num; ++i)
	    {
            ...
            sprintf(zk_path, "twemproxy/%s/nodes/%s", product_prefix, strings.data[i] );
	    ...
	    if (backupnode != NULL)
	    {
		    ...
		    char data[1024] = {0};
		    char zk_path[ZK_PATH_LEN] ={0};
		    sprintf(zk_path,"/twemproxy/%s/nodes/%d" ,product_prefix, atoi(strings.data[nodes_num-1]) + 1 );
...
int main(int argc, char  **argv)
{
    ...
    //
    int nodes_num = add_node(zh_handler, host, migrate.product_prefix, migrate.backupnode);
    if (nodes_num < 0 )
    {
	...
	migarte_status(zh_handler, nodes_num, "true",  vecSlots, migrate.product_prefix);
    char data[1024];
    int datalen = sizeof(data);
    char zk_path[ZK_PATH_LEN];
	...
	for (std::vector<std::string>::iterator it = vecSlots.begin() ; it != vecSlots.end(); it++ )
	{
		...
		if(zh_handler)
		{
            ...
            sprintf(zk_path, "/twemproxy/%s/slot_map/%s", migrate.product_prefix, (*it).c_str() );
            ...
            memset(data,0x00, sizeof(data));
            memset(zk_path,0x00, sizeof(pathlen));
            sprintf(zk_path, "/twemproxy/%s/nodes/%d", migrate.product_prefix, node_index );
		...
		memset(zk_path, 0x00, sizeof(zk_path));
		sprintf(zk_path, "/twemproxy/%s/migrate_tasks/%s", migrate.product_prefix, (*it).c_str());
