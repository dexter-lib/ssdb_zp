...
def set_slave(master_ip, master_port, slave_ip, slave_port):
    try:
        ...
        err_str = "set slave ssdb %s:%d connected error:%s, "%(slave_ip, slave_port, str(e))
        handle_output(False, err_str)
...
def get_next_node_index(zk, product_prefix):
    try:
        zk_path = "/twemproxy/%s/nodes"%product_prefix
        zk_children = zookeeper.get_children(zk, zk_path)
        max_index = 0
        ssdb_groups = []
        for tp in zk_children:
            index = int(tp)
...
def add_node(path_str, zookeeper_str, product_prefix):
    ...
    if ret == False:
        ...
        return False
    zk = zookeeper.init(zookeeper_str)
    node_index, old_ssdb_groups = get_next_node_index(zk, product_prefix)
    if node_index < 0:
    ...
    for s in ssdb_groups:
        ...
        path = "/twemproxy/%s/nodes/%d"%(product_prefix, node_index)
        create_zookeeper(zk, path, data)
...
def init_all_node(path_str, zookeeper_str, product_prefix):
    ret, ssdb_groups = parse_ssdb_group(path_str)
    if ret == False:
    ...
    zk = zookeeper.init(zookeeper_str)
    create_zookeeper_without_delete_children(zk, "/twemproxy", "")
    zk_path = "/twemproxy/%s"%(product_prefix)
    create_zookeeper(zk, zk_path, "1")
    zk_node_path = zk_path + "/nodes"
    create_zookeeper(zk, zk_node_path, "1")
    zk_slot_map_path = zk_path + "/slot_map"
    create_zookeeper(zk, zk_slot_map_path, "1")
    ...
    for s in ssdb_groups:
        ...
        init_ssdb(s.ip, s.port)
        #init_ssdb(s.slave_ip, s.slave_port)
        path = zk_node_path + "/" + str(node_index)
        create_zookeeper(zk, path, data)
        node_index = node_index + 1
    init_slot_map(zk, node_size, product_prefix)

def create_slotmap_file(pathstr, zookeeper_str, product_prefix):
    try:
        ...
        for i in range(16384):
            slotmap_path = "/twemproxy/%s/slot_map/%d"%(product_prefix, i)
...
def get_ssdb_mem_info(zookeeper_str, product_prefix):
    path = "/twemproxy/%s/nodes"%product_prefix
    mem_info = "unknown"
...
def main():
    ...
    parser.add_option("-P","--product_prefix",action="store",type="string",dest="product_prefix",help="product line")
    parser.add_option("-t","--twemproxy_host",action="store",type="string",dest="twemproxy_host",help="twemproxy_host")
    (options, args) = parser.parse_args()
    #pdb.set_trace()
    if options.init_file_path and options.zookeeper_str and options.product_prefix: 
        ...
        if not os.path.exists(options.init_file_path):
            ...
            return
        init_all_node(init_file_path, zookeeper_str, product_prefix)
    elif options.add_node_file_path and options.zookeeper_str and options.product_prefix: 
        ...
        if not os.path.exists(add_node_file_path):
            ...
            handle_output(False, err_str)
            return
        if add_node(add_node_file_path, zookeeper_str, product_prefix):
            handle_output(True)
    elif options.create_node_file_path and options.zookeeper_str and options.product_prefix:
        ...
        if not os.path.isdir(create_node_file_path):
            ...
            return
        create_slotmap_file(create_node_file_path, zookeeper_str, product_prefix)
    elif options.model_str and options.zookeeper_str and options.product_prefix:
        model_str             = options.model_str
        zookeeper_str         = options.zookeeper_str
        product_prefix        = options.product_prefix
        if model_str == "ssdb_mem_info":
            get_ssdb_mem_info(zookeeper_str, product_prefix)
        if model_str == "add_twemproxy":
            if options.twemproxy_host and options.port:
                if not add_twemproxy(options.twemproxy_host, options.port, zookeeper_str, product_prefix):
