...
//      offsetof(struct conf_pool, master) },
    { string("product_prefix"),
      conf_set_string,
      offsetof(struct conf_pool, product_prefix) },
        ...
        void *watcherCtx)
{
    ...
    if (type == ZOO_CHILD_EVENT) {
        ...
        snprintf(zk_path, path_len, "/twemproxy/%s/nodes", (char *)sp->product_prefix.data);
        int child_ret = zk_get_children(zh, zk_path, nodes_child_watch, watcherCtx, &strings);
        if (child_ret) {
            return;
        }
...
void
node_get_watch(zhandle_t *zh, int type, int state, const char *path,
        void *watcherCtx)
{
    log_debug(LOG_DEBUG, "call node_get_watch");
    struct server * s;
    int    server_index;
    char   *server_index_str;
    int    old_value;

    if (type == ZOO_CHANGED_EVENT || type == ZOO_DELETED_EVENT) {
        struct server_pool *pool = (struct server_pool *)watcherCtx;
        char data[128];
        int datalen = sizeof(data);
        memset(data, 0, datalen);
        int get_ret = zk_get(zh, path, node_get_watch, watcherCtx, data, &datalen);
        if (get_ret) {
            return;
        }

        if(!pool->finish_init) {
            log_debug(LOG_DEBUG, "server_pool incomplete initialization");
            return;
        }

        server_index_str = basename(path);
        server_index     = atoi(server_index_str);
        s = array_get(&pool->server, server_index);

        if(!s->active_standby_switch)
            __sync_bool_compare_and_swap(&(s->active_standby_switch), old_value, 1);
        else
            log_debug(LOG_DEBUG, "server_pool incomplete active standby switch");
    } else {
        return;
    }
}
...
conf_from_zookeeper(zhandle_t *zh_handler, struct array * servers, struct array *backup_servers, struct server_pool *sp)
{
    ...
    int  path_len = sizeof(zk_path);
    snprintf(zk_path, path_len, "/twemproxy/%s/nodes", (char *)sp->product_prefix.data);
    int child_ret = zk_get_children(zh_handler, zk_path, nodes_child_watch, sp, &strings);
    if (child_ret) {
        return NC_ERROR;
    } else {
        ...
        if (child_num == 0) {
            ...
            for (zk_index = 0; zk_index < child_num; ++zk_index) {
                ...
                sprintf(zk_path, "/twemproxy/%s/nodes/%s", (char *)sp->product_prefix.data, strings.data[zk_index]);
                int get_ret = zk_get(zh_handler, ((const char *)zk_path), node_get_watch, sp, data, &datalen);
                if (get_ret) {
                    return NC_ERROR;
                }
...
static void *
check_zookeeper_master(void *p)
{
    char tmp_path[64];

    if(!p) return (void *)NULL;

    struct server_pool *pool = (struct server_pool *)p;

    char zk_path[ZK_PATH_LEN];
    snprintf(zk_path, ZK_PATH_LEN, "/twemproxy/%s", (char *)pool->product_prefix.data);
    ...
    while(true)
    {
        if(!pool->master)
        {
            ...
            if(get_lock(pool->zh_handler, zk_path, tmp_path, 1000))
            {
                pool->master = true;
                __sync_bool_compare_and_swap(&(pool->master), pool->master, 1);
                log_debug(LOG_VERB, "node is master");
            }
        }
        sleep(10);
    }
...
conf_pool_each_transform(void *elem, void *data)
{
    ...
    sp->product_prefix = cp->product_prefix;
    ...
    if (array_n(&cp->zookeeperserver) != 0) {
        ...
        status = pthread_create(&sp->tid, NULL, check_zookeeper_master, sp);
        if (status < 0) {
            log_error("check master thread create failed: %s", strerror(status));
            return NC_ERROR;
        }
    }
