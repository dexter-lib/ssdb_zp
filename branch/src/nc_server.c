...
server_each_connected_determine(void *elem, void *data)
{
    ...
    if(server->next_retry > now) {
        ...
        if (status != NC_OK) {
            log_warn("connect to server '%.*s' failed, ignored: %s, status:%d",
                     server->pname.len, server->pname.data, strerror(errno), status);
...
static void * connected_determine_thread(void *t){
    ...
    int64_t now;
    rstatus_t status;
    struct determine_server *ds;
    struct string server;
    int port;
    bool check;
    int64_t new_time, old_time;

    ds = (struct determine_server *)t;
    server.data = (char *)ds->slave_addrstr.data;
    server.len  = ds->slave_addrstr.len;
    ...
    while(check) {
        ...
        //if(*ds->next_retry > now)
        {
            old_time = *ds->next_retry;
            status = connect_determine(&server, ds->slave_port);
            log_warn("try to connect '%.*s': %d", server.len, server.data, ds->slave_port);
            if(NC_OK != status){
                 new_time = now + ds->retry_timeout;
                 //__sync_bool_compare_and_swap(ds->next_retry, old_time, new_time);
                sleep(2);
                continue;
            }else{
                check = false;
                //__sync_bool_compare_and_swap(ds->next_retry, old_time, 0);
            }

            if(ds->is_master)
            {
                if(ds->ssdb_handle) {
                    lib_ssdb_change_master_to_t change_master_to = (lib_ssdb_active_standby_switch_t)dlsym(ds->ssdb_handle, "change_master_to");
                    int err_no = change_master_to((const char*)ds->slave_addrstr.data, ds->slave_port, ds->last_seq,
                                        (const char *)ds->master_addrstr.data, ds->master_port);
                    log_warn("change_master_to %s:%d to master:%s, %d, last_seq:%ld",
                            (const char*)ds->master_addrstr.data, ds->master_port, (const char *)ds->master_addrstr.data, ds->master_port,ds->last_seq);
                }else{
                    log_warn("ssdb handle is NULL");
                    return NC_ERROR;
                }
...
static rstatus_t
server_each_active_standby_switch(void *elem, void *data){
    rstatus_t status;
    struct server *server, *master_server;
    struct server_pool *pool;
    ...
    if(!server->active_standby_switch) return NC_OK;
    ...
    server->next_retry = now + pool->server_retry_timeout;
    status = server_memory_active_standby_switch(server);
    log_warn("server_memory active standby switch");

    old_value = server->active_standby_switch;
    __sync_bool_compare_and_swap(&(server->active_standby_switch), old_value, 0);
    return status;
}
...
server_close(struct context *ctx, struct conn *conn)
{
    ...
    if(!s->ns_conn_q){
        server_zookeeper_active_standby_switch(s);
    }
}
...
rstatus_t
connect_determine(const struct string *server, int port)
{
    rstatus_t status;
    fd_set  readfds, writefds, exfds;
    struct timeval tv;
    ...
    log_debug(LOG_VVERB, "connect to server '%.*s'", server->len, (char *)server->data);

    //conn->sd = socket(conn->family, SOCK_STREAM, 0);
    fd = socket(AF_INET, SOCK_STREAM, 0);

    if (fd < 0) {
        log_error("socket for server '%.*s' failed: %s", server->len,
                  (char *)server->data, strerror(errno));
        status = NC_ERROR;
        goto error;
    }
    ...
    if (status != NC_OK) {
        log_error("set nonblock on s %d for server '%.*s' failed: %s",
                  fd, server->len, (char *)server->data,
                  strerror(errno));
        goto error;
    }
    ...
    if (server->data[0] != '/') {
        ...
        if (status != NC_OK) {
            log_warn("set tcpnodelay on s %d for server '%.*s' failed, ignored: %s",
                     fd, server->len, server->data,
                     strerror(errno));
        }
    }


    sin_in = (struct sockaddr_in*)&addr_to;
    sin_in->sin_addr.s_addr = inet_addr((char *)server->data);
    sin_in->sin_port        = htons(port);
    sin_in->sin_family      = AF_INET;
    sock_len                = (socklen_t)sizeof(struct sockaddr_in);

    status = connect(fd, &addr_to, (socklen_t)(sizeof(struct sockaddr)));
    if (status != NC_OK) {
        if (errno == EINPROGRESS) {
//            conn->connecting = 1;
//            log_debug(LOG_DEBUG, "connecting on s %d to server '%.*s'",
//                      conn->sd, server->pname.len, server->pname.data);
            ...
            int ret = select(fd + 1, &readfds, &writefds, &exfds, &tv);
            if(ret <= 0){
                log_error("connect on s %d to server '%.*s' failed: %s", fd,
                          server->len, server->data, strerror(errno));
                status = NC_ERROR;
                goto error;
            }else{
                status = getpeername(fd, &addr_to, &sock_len);
                if (status != NC_OK) {
                    perror("getpeername:");
                    goto error;
                }
...
static rstatus_t
server_pool_each_active_standby_switch(void *elem, void *data)
{
    rstatus_t status;
    struct server_pool *sp = elem;

    status = array_each(&sp->server, server_each_active_standby_switch, NULL);
    if (status != NC_OK) {
        return status;
    }
...
rstatus_t
server_pool_active_standby_switch(struct context *ctx)
{
    rstatus_t status;
    status = array_each(&ctx->pool, server_pool_each_active_standby_switch, NULL);
    return NC_OK;
}
...
server_memory_active_standby_switch(struct server *server)
{
    ...
    int  path_len = sizeof(zk_path);
    snprintf(zk_path, path_len, "/twemproxy/%s/nodes", (char *)pool->product_prefix.data);
    int child_ret = zk_get_children(pool->zh_handler, zk_path, NULL, NULL, &strings);
    if (child_ret) {
        return NC_ERROR;
    } else {
...
//        server = tmp_server;
    ...
    if(!pool->master)
    {
        log_warn("[server_memory_active_standby_switch]not master return");
        return NC_OK;
    }



    //ssdb active_standby_switch only one
    if(pool->ssdb_handle) {
    ...
    //server->next_retry = now + pool->server_retry_timeout;
    struct determine_server *ds = malloc(sizeof(struct determine_server));
    ds->next_retry = &server->next_retry;
    ds->master_addrstr.data = server->addrstr.data;
    ds->master_addrstr.len  = server->addrstr.data;
    ds->master_port         = server->port;
    ds->is_master    = pool->master;
    ds->slave_addrstr.data = backup_server->addrstr.data;
    ds->slave_addrstr.len = backup_server->addrstr.len;
    ds->slave_port  = backup_server->port;
    ds->last_seq     = pool->last_seq;
    ds->retry_timeout = pool->server_retry_timeout;
    ds->ssdb_handle   = pool->ssdb_handle;

    pthread_t pthid;
    pthread_create(&pthid, NULL, connected_determine_thread, ds);
...
rstatus_t
server_zookeeper_active_standby_switch(struct server *server)
{
    struct server *backup_server, tmp_server;
    int64_t now;                  /* current timestamp in usec */
    struct server_pool *pool;     /* server pool */
...
//        TAILQ_INIT(&backup_server->s_conn_q);
        ...
        if(!pool->master)
        {
            log_warn("[server_zookeeper_active_standby_switch]not master return");
            return NC_OK;
        }
...
//        }
        ...
        server_ip = backup_server->addrstr;
        back_ip   = server->addrstr;
        server_port = backup_server->port;
        back_port = server->port;
        ...
        sprintf(zk_path, "/twemproxy/%s/nodes/%s", (char *)pool->product_prefix.data, strings.data[server_index]);
