...
struct server {
    ...
    int                active_standby_switch; /*active_standby_switch flag*/
...
struct server_pool {
    ...
    uint64_t           last_seq;
    pthread_t          tid;                  /* check master thread id */
};


struct determine_server {
    struct string  slave_addrstr;
    int     slave_port;
    struct string  master_addrstr;
    int     master_port;
    int64_t *next_retry;
    int64_t last_seq;
...
rstatus_t connect_determine(const struct string *server, int port);
...
rstatus_t server_pool_active_standby_switch(struct context *ctx);
...
rstatus_t server_zookeeper_active_standby_switch(struct server *server);
rstatus_t server_memory_active_standby_switch(struct server *server);
rstatus_t server_crash(struct server *server);
