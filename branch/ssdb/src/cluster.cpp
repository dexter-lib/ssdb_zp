...
int SSDBCluster::init() {
	...
	memset(importing_slots, 0, sizeof(importing_slots));
	if(is.size() == sizeof(importing_slots)) {
		memcpy(importing_slots, is.c_str(), is.size());
		log_debug("importing_slots ok");
	} else {
        log_error("importing_slots error, save size %u, importing_slots size %u",
                is.size(), sizeof(importing_slots));
	}
