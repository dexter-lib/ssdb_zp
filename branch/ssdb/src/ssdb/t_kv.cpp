...
KIterator* SSDBImpl::scan(const Bytes &start, const Bytes &end, uint64_t limit){
	...
	if (start.empty()) {
		...
		ret = this->get_version(start, &t, &start_version);
		if (ret == -1 || ret == 0) {
			return NULL;
		}
		key_start = encode_kv_key(start, start_version);
	}
	if (end.empty()) {
		key_end = "";
	}
	else {
		ret = this->get_version(end, &t, &end_version);
		if (ret == -1 || ret == 0) {
			key_end = "";
		}
		else {
			key_end = encode_kv_key(end, end_version);
		}
	}
